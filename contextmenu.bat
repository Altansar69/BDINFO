...
REG ADD HKEY_CURRENT_USER\Software\Classes\%ProgID%\shell\BDNFO /v icon /d "%cd%\test.exe,0" /f
REG ADD HKEY_CURRENT_USER\Software\Classes\%ProgID%\shell\BDNFO\command /f
REG ADD HKEY_CURRENT_USER\Software\Classes\%ProgID%\shell\BDNFO\command /ve /d "%cd%\test.exe """%%1"""" /f
goto end
